$(document).ready(function(){
    ...
    function handleInput(input) {
      // if (input === "0" && display.text() === "") return;
    ...
    //   var removeChar = display.text().replace(/[^0-9\.]/g, '') // This is to remove alphabets and special characters.
    //   // console.log(removeChar);
    //   var removeDot = removeChar.replace(/\./g, '') // This is to remove "DOT"
    //   display.text(removeDot);

     
    // var d = display.text().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    // display.text(d);
      ...
      //   input === '*' ||
      //   input === '/' ||
      //   input === '%')) { // ignore if input is prohibited operator and display is empty
      ...
      // }
        switch (input) {
            ...
            default:
                ...
                if (display.text().length >= 20) { // set the limit to 30 digits
                    break;
                }
